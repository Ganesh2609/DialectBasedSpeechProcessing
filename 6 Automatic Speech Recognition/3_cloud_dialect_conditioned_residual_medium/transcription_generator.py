"""
Transcription Generator - Single Sample Inference (Cloud Training)
...
def main():
    ...
    CHECKPOINT_PATH = f"{BASE_PATH}/train_data/checkpoints/best_model.pth"
    OUTPUT_PATH = f"{BASE_PATH}/train_data/transcriptions_comparison.csv"
    
    # Audio path conversion for cloud
    ...
    def convert_audio_path(audio_path):
        ...
        if "Final Dataset" in audio_path:
            ...
            audio_path = f"{AUDIO_BASE_PATH}/Final Dataset{relative_part}"
    ...
    with torch.no_grad():
        for idx in tqdm(val_indices, desc="Generating"):
            row = df.iloc[idx]
            audio_path = convert_audio_path(row['audio_path'])
            audio_id = row['name']
