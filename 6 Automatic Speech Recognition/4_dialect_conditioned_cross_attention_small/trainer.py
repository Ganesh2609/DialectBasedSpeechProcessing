"""
Trainer for Dialect-Conditioned ASR (Cross-Attention)

Key features:
- Staged training: warmup (projection only) → fine-tune (decoder + projection)
...
class DialectConditionedASRTrainer:
    """
    Trainer for dialect-conditioned Whisper ASR with staged training.
    ...
    def __init__(
        ...
        projection_warmup_epochs: Optional[int] = 3,  # NEW: warmup epochs
    ...
    def train_epoch(self) -> None:
        ...
        with tqdm(enumerate(self.train_loader), total=len(self.train_loader), 
            ...
            for i, batch in t:
                ...
                dialect_embedding = batch['dialect_embedding'].to(self.device)
    ...
    def train(self, resume_from: Optional[str] = None) -> None:
        """Run full training loop with staged training."""
        ...
        if resume_from:
            ...
            self.logger.info(f"Projection warmup epochs: {self.projection_warmup_epochs}")
    
        print(f"Starting training from epoch {self.current_epoch} to {self.num_epochs}")
        print(f"Projection warmup epochs: {self.projection_warmup_epochs}")

        for epoch in range(self.current_epoch, self.num_epochs + 1):
            self.current_epoch = epoch
            
            # Handle staged training transitions
            if epoch <= self.projection_warmup_epochs:
                # Phase 1: Warmup - train projection only
                if epoch == 1 or (epoch == self.current_epoch and not hasattr(self, '_warmup_set')):
                    self.model.set_warmup_mode(True)
                    self._warmup_set = True
                    self.logger.info(f"=== PHASE 1: Warmup (epochs 1-{self.projection_warmup_epochs}) ===")
                    self.logger.info("Training projection layer only, Whisper frozen")
                    print(f"\n=== PHASE 1: Warmup (epochs 1-{self.projection_warmup_epochs}) ===")
            else:
                # Phase 2: Fine-tune - train decoder + projection
                if epoch == self.projection_warmup_epochs + 1:
                    self.model.set_warmup_mode(False)
                    self.logger.info(f"=== PHASE 2: Fine-tuning (epochs {self.projection_warmup_epochs + 1}-{self.num_epochs}) ===")
                    self.logger.info("Training decoder + projection, encoder frozen")
                    print(f"\n=== PHASE 2: Fine-tuning (epochs {self.projection_warmup_epochs + 1}-{self.num_epochs}) ===")
    ...
    def save_checkpoint(self, is_best: Optional[bool] = False):
        ...
        checkpoint = {
            ...
            'projection_warmup_epochs': self.projection_warmup_epochs,
        }
    ...
    def load_checkpoint(self, checkpoint: Optional[str] = None, resume_from_best: Optional[bool] = False):
        ...
        self.projection_warmup_epochs = checkpoint.get('projection_warmup_epochs', self.projection_warmup_epochs)
